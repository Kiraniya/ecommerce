...
def getSellerLoginDetails():
    with sqlite3.connect('database.db') as conn:
        ...
        if 'email' not in session:
            ...
            loggedIn = True
            cur.execute("SELECT SellerID, name FROM seller WHERE email = ?", (session['email'], ))
            SellerID, Name = cur.fetchone()
    conn.close()
...
def addItem():
    if request.method == "POST":
    	sellerid, sname = getSellerLoginDetails()
...
def iss_valid(email, password):
    con = sqlite3.connect('database.db')
    cur = con.cursor()
    cur.execute("SELECT Password FROM seller where email = '"+email+"'")
    data = cur.fetchall()
    for row in data:
    	if row[0] == password:
            return True
...
def sellerproductDescription():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        productData = cur.fetchone()
    conn.close()
    return render_template("sellerproductdescription.html", data=productData, Name = name)
